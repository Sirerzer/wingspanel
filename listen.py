...
app.secret_key = ''
...
def echo(ws, uuid):
    logging.debug(f"New WebSocket connection with UUID: {uuid}")
    try:
        while not ws.closed:
            message = ws.receive()
            if message:
...
def run(debug=True, port=5000, certfile=None, keyfile=None):
